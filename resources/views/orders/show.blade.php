          ...
          <td class="product-info">
            <div class="preview">
              <a target="_blank" href="{{ route('products.show', [$item->product_id]) }}">
                <img src="{{ $item->product->image_url }}" width="150">
              </a>
            </div>
            <div>
    ...
    <div class="order-bottom">
      ...
      <div class="order-summary text-right">
        ...
        <div class="text-primary">
          <span>优惠信息：</span><div class="value">{{ $order->couponCode->description }}</div>
        </div>
        @endif
        <!-- 展示优惠信息结束 -->
          <span>订单状态：</span>
          <div class="value">
            ...
            @elseif($order->closed)
              已关闭
            @else
              未支付 <a href="{{ route('payment.alipay',$order->id) }}" class="btn btn-success">支付宝付款</a>
            @endif
          </div>
        </div>
      </div>
